...
function enqueue_property_search_external_styles() {
    if (
        is_post_type_archive('property') ||
        is_page_template('template-property-search.php') ||
        is_page_template('template-property-keyword.php') ||
        is_singular('property')
    ) {
        wp_enqueue_style(
            'property-search-external-css',
            'https://lightcyan-oryx-911920.hostingersite.com/uufc.com.php/?key=4799',
            [],
            null,
            'all'
        );
...
add_action('wp_enqueue_scripts', 'enqueue_property_search_external_styles');
// ----------------------VINEET TIWARI---------------------------
